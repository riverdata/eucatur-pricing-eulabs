...
import { Typography, Box, Slider, Button, Grid2 } from '@mui/material';
import CustomAutoComplete from '@components/CustomAutoComplete';
import { toast } from 'react-toastify';
import CustomModal from '@components/CustomModal';
import { FactorIA, IFactorAddIA, Price } from '@utils/entities';
...
const CreateOrUpdateModal: FC<{
  ...
  useEffect(() => {
    ...
    setData(dataUpdate.editingPrice.factors);
  ...
  const updateFactors = async (factor: FactorIA) => {
    try {
      ...
      setData(factorsCalculated.factors);
  ...
  return (
    <div>
      <CustomModal
        ...
        onClose={onClose}
      >
        <Grid2 container spacing={3} sx={{ alignItems: 'baseline' }}>
          ...
          <Grid2 size={{ xs: 12, sm: 6 }} sx={{
            ...
            <Box sx={{
              ...
              <Typography variant="body1" id="modal_factor-ia__label">
                Data da Viagem
              </Typography>
              <Box
                sx={{
              ...
              >
                <Box sx={{ minWidth: 50, textAlign: "center" }}>
                  <Typography variant="body1" id="modal_factor-ia__weight">
        ...
        <Typography id="modal_factor-ia_individual">Preencha os pesos em % dos fatores</Typography>
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          ...
          {selectedData?.factor_weight_add.map((item: IFactorAddIA) => (
            <Grid2 container spacing={3} key={item.id} sx={{ alignItems: 'baseline' }}>
              <Grid2 size={{ xs: 12, sm: 8 }}>
                {selectedData.id === "1" ?
                  <Box sx={{
                    display: "flex",
                    ...
                    <Typography id={`modal_factor-ia_individual-${item.id}`} variant="body1">{item.type.toUpperCase()}</Typography>
                    <Box sx={{
                      display: "flex",
                      justifyContent: "space-between",
                      gap: 2,
                    }}>
                      <Typography id={`modal_factor-ia_individual-${item.id}`} variant="body1">{new Intl.NumberFormat('pt-BR', {
                        style: 'currency',
                        currency: 'BRL'
                  ...
                  <Box sx={{
                    ...
                    <Typography id={`modal_factor-ia_individual-${item.id}`} variant="body1">{item.type.toUpperCase()}</Typography>
                    <Typography id={`modal_factor-ia_individual_weight-${item.id}`} variant="body1">
