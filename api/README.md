...
Configure o Aplicativo EUCATUR PRICING API seguindo os passos a seguir: (Precisa configurar o DNS para funcionar)
![image](https://github.com/user-attachments/assets/363cbcf4-9c85-462d-9e82-b3ea8d61efae)

![image](https://github.com/user-attachments/assets/e50c121d-9d40-4089-a201-eb8b6ca491b1)

</details>

<details>
<summary>## ✨ Procedimento para iniciar o banco de dados e o servidor Nginx</summary>

1. **Criar a pasta do projeto**  
   No servidor, crie o diretório que armazenará os arquivos do projeto:

```bash
   mkdir eucatur-pricing-api
   cd eucatur-pricing-api
```

2. **Copiar os arquivos do projeto para o servidor**
Transfira os arquivos da pasta local eucatur-pricing-api para o diretório eucatur-pricing-api no servidor.

Observação: compile o projeto previamente e envie apenas o conteúdo da pasta dist.

3. **Criar pastas auxiliares**
No diretório do projeto, crie as pastas necessárias para armazenamento de dados e certificados:
```bash
   mkdir letsencrypt data database_plataforma_eucatur
```

4. **Iniciar os serviços**
Execute o comando abaixo para subir o banco de dados e o servidor Nginx utilizando o docker-compose específico da plataforma:
```bash
sudo docker-compose -f docker-compose-plataforma_eucatur up -d
```

</details>

<details>
<summary>## ✨ Execução das migrações do banco de dados</summary>
...
Após a instalação, rode o script responsável por aplicar as migrações do banco de dados em JavaScript:

```bash
sudo npm run migration:run-js
```
</details>
...
```bash
sudo docker build . -t eucatur-pricing-api
```
